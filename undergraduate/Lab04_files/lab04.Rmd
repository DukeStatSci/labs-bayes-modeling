---
title: "STA 360 Lab 5: Introduction to Hamiltonian Monte Carlo"
...
Now that we have derived the full condition densities, we can write some code to implement a Gibbs sampler for this model. Specifically, we write a function called `normal_gibbs_sampler` that takes as arguments (1) the number of samples desired (`S`), (2) an $n \times 2$ matrix of data values (`X`), and (3) the given correlation parameter (`rho`).
