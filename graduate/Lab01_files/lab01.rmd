...
We will frequently work with vectors and matrices. Vectors can be assigned using the c() function (I believe the 'c' stands for 'combine'). We can apply element-wise arithmetic to numeric vectors as follows. Make sure that you understand the following operations!
...
You may be accustomed to extract a column of data using the dollar sign operation. We can also use tidyverse language to access/manipulate data frames. However, what is returned from the select() function is not a numeric vector but another data frame:
...
str(bp2)
```
In this course we will almost exclusively work with numerical data. Therefore, you may not need to foray into data frames. However, learning the tidyverse syntax can be extremely beneficial as you move forward in your statistical career.
